...
const config = {
  ...
  plugins: [
    ...
    plugin(function ({ addUtilities, theme, escapeSelector }) {
      const colors = theme("colors");
      const filterDefault = str => str.replace(/[^a-zA-Z0-9]+/g, '-');

      const generateUtilities = (colors, prefix = "") => {
        return Object.keys(colors).reduce((acc, colorName) => {
          const colorValue = colors[colorName];
          const className = prefix ? `${prefix}-${filterDefault(colorName)}` : filterDefault(colorName);
