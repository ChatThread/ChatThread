...
export function AuthProvider({ children }): React.ReactElement {
  ...
  const [accessToken, setAccessToken] = useState<string | null>(
    cookies.get(CHATMAGIC_ACCESS_TOKEN) ?? null,
  );
  const [userData, setUserData] = useState<Users | null>(null);
  const [apiKey, setApiKey] = useState<string | null>(
    cookies.get(CHATMAGIC_API_TOKEN),
  );
  ...
  useEffect(() => {
    const storedAccessToken = cookies.get(CHATMAGIC_ACCESS_TOKEN);
  ...
  useEffect(() => {
    const apiKey = cookies.get(CHATMAGIC_API_TOKEN);
    if (apiKey) {
      setApiKey(apiKey);
    }
  }, []);
  ...
  function login(
    ...
    cookies.set(CHATMAGIC_AUTO_LOGIN_OPTION, autoLogin, { path: "/" });
    if (refreshToken) {
      cookies.set(CHATMAGIC_REFRESH_TOKEN, refreshToken, { path: "/" });
    }
