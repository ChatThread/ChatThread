...
export interface INewChatSession {
  id: string
  flow_id: string
  /** optional namespace to separate different pages (e.g. 'playground' vs 'new-chat') */
  namespace?: string
  /** optional display name for the session (separate from the internal id) */
  name?: string
  created_at: number
  updated_at: number
}

export interface INewChatMessage extends Message {
  /** optional namespace to separate different pages (e.g. 'playground' vs 'new-chat') */
  namespace?: string
  created_at: number
  updated_at: number
