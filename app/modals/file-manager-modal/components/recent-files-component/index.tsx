...
import { sortByBoolean, sortByDate } from "@/pages/main-page/utils/use-sort-flows";
import { FileType } from "@/types/file-management";
import Fuse from "fuse.js";
import { useEffect, useMemo, useState } from "react";
import FilesRendererComponent from "../files-renderer-component";
...
export default function RecentFilesComponent({
  ...
  const filesWithType = useMemo(() => files.map((file) => ({
    ...file,
  ...
  useEffect(() => {
    if (filesWithType?.length) {
      setFuse(new Fuse(filesWithType, {
        keys: ["name", "type"],
        threshold: 0.3,
      }),
      );
  ...
  return (
    <div className="h-full flex flex-col gap-4 overflow-hidden">
      ...
      <div
        className={`flex-1 flex-col gap-1 overflow-y-auto overflow-x-hidden`}
      >
