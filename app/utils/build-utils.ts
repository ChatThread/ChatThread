import { MISSED_ERROR_ALERT } from "@/constants/alerts-constants";
import {
  BASE_URL_API,
  POLLING_INTERVAL,
  POLLING_MESSAGES,
...
import { isErrorLogType } from "../types/utils/type-checking-utils";
...
async function pollBuildEvents(
  ...
  while (!isDone) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.detail ||
          "ChatThread was not able to connect to the server. Please make sure your connection is working properly.",
      );
    }
...
export async function buildFlowVertices({
  ...
  try {
    ...
    const host = (import.meta.env.VITE_API_BASE_URL as string) || "http://localhost:7860";
    buildUrl=host+buildUrl
    ...
    const cancelBuildUrl = host+`${BASE_URL_API}build/${job_id}/cancel`;
    ...
    const eventsUrl = host + `${BASE_URL_API}build/${job_id}/events`;
